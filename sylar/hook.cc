...
namespace sylar {
	...
	void hook_init() {
	  ...
	  //获取name函数的地址，并将其赋予name_f变量，完成对name函数的hook。
	  //这样，调用name函数会调用自己定义的函数，而非库函数，同时也将原函数定义到name_f
	...
	struct _HookIniter {
		_HookIniter() {
			hook_init();
			s_connect_timeout = sylar::g_tcp_connect_timeout->getValue();
...
//HOOKsocketIO事件，如果未定义HOOK，则执行原函数，如果HOOK
	//则重复执行socketIO操作，直至出现超时的情况。
		...
		if(n == -1 && errno == EAGAIN) { //当前资源暂不可用
			...
			if(to != (uint64_t)-1) {
				timer = iom->addConditionTimer(to, [winfo, fd, iom, event]() {
					auto t = winfo.lock(); //.lock()方法返回一个shared_ptr对象
					if(!t || t->cancelled) {
						return;
					}
...
extern "C" {
  ...
  int connect_with_timeout(int fd, const struct sockaddr* addr, socklen_t addrlen, uint64_t timeout_ms) {
  	...
  	else if(n != -1 || errno != EINPROGRESS) { //EINPROGRESS表示阻塞操作正在进行
  		return n;
  	}
