...
namespace sylar {
	...
	public:
		...
		typedef std::function<void (const T& old_value, const T& new_value)> on_change_cb; // 函数返回值(形参列表))
	...
	public:
		...
		//带参数的查找，如果在map中为查询到name的键值对,创建一个<string, Configvar::ptr>类型的键值对，并添加。
		typedef RWMutex RWMutexType;
		template<class T>
		static typename ConfigVar<T>::ptr Lookup(const std::string& name,
			const T& default_value, const std::string& description = "") {
				...
				if(it != s_datas.end()) {
					auto tmp = std::dynamic_pointer_cast<ConfigVar<T>>(it->second); //dynamic_pointer_cast：将基类的智能指针安全地转换成派生类的指针
					if(tmp) {
				...
				if(name.find_first_not_of("abcdefghijklmnopqrstuvwxyz._012345678") //查找name中第一个不是()中元素的下标位置
							!= std::string::npos) {																																 //如果没找到，会返回string::npos
