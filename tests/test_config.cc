...
void test_yaml() {
    YAML::Node root = YAML::LoadFile("/home/test01/sylar/bin/conf/test.yml");
    print_yaml(root, 0);
...
public:
	...
	bool operator==(const Person& p2) const{
		return p2.m_name == m_name && p2.m_age == m_age && p2.m_sex == m_sex;
	}
};
...
void test_log() {
	...
	YAML::Node root = YAML::LoadFile("/home/test01/sylar/bin/conf/log.yml");
	sylar::Config::LoadFromYaml(root);
	std::cout << "========================" << std::endl;
	std::cout << sylar::LoggerMgr::GetInstance()->toYamlString() << std::endl;
	SYLAR_LOG_INFO(system_log) << "hello system" << std::endl;
