...
void run() {
	sylar::Address::ptr addr = sylar::Address::LookupAnyIPAddress("www.sylar.com:80");
	if(!addr) {
	...
	if(!rt) {
		SYLAR_LOG_INFO(g_logger) << "connect " << *addr << " faild";
		return;
	}
	...
	auto r = sylar::http::HttpConnection::DoGet("http://www.sylar.com/blog/", 300);
